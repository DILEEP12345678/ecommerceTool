  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-5 pb-10 space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <KpiCard label="Confirmed"     value={metrics.confirmed} icon={Clock}        color="bg-amber-500"   textColor="text-white" subColor="text-amber-100"  iconColor="text-amber-100" />
        <KpiCard label="Packed"        value={metrics.packed}    icon={Package}      color="bg-blue-600"    textColor="text-white" subColor="text-blue-100"   iconColor="text-blue-100" />
        <KpiCard label="Collected"     value={metrics.collected} icon={CheckCircle2} color="bg-green-600"   textColor="text-white" subColor="text-green-100"  iconColor="text-green-100" badge={`${metrics.fulfillmentRate}% rate`} />
      </div>

      {/* ── KPI CARDS ROW 2 — Business health ────────────── */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <AovCard avgOrderValuePence={metrics.avgOrderValuePence} />
        <KpiCard label="Active Customers" value={metrics.activeCustomers}  icon={Users}       color="bg-violet-600"  textColor="text-white" subColor="text-violet-100"  iconColor="text-violet-100" />
        <KpiCard label="Repeat Customers" value={metrics.repeatCustomers}  icon={RefreshCw}   color="bg-indigo-600"  textColor="text-white" subColor="text-indigo-100"  iconColor="text-indigo-100" badge={`${metrics.repeatRate}% rate`} />
...
function RevenueCard({ revenuePence }: { revenuePence: number }) {
  ...
  return (
    <div className="col-span-2 lg:col-span-1 rounded-2xl p-4 flex flex-col gap-2 bg-primary-600" style={{ boxShadow: '0 1px 2px rgba(0,0,0,0.2), 0 2px 6px rgba(0,0,0,0.12)' }}>
...
function KpiCard({ label, value, icon: Icon, color, textColor, subColor, iconColor, badge }: {
  ...
  return (
    <div className={`rounded-2xl p-4 flex flex-col gap-2 ${color}`} style={{ boxShadow: '0 1px 2px rgba(0,0,0,0.2), 0 2px 6px rgba(0,0,0,0.12)' }}>
      <div className="flex items-center justify-between">
...
function AovCard({ avgOrderValuePence }: { avgOrderValuePence: number }) {
  ...
  return (
    <div className="col-span-2 lg:col-span-1 rounded-2xl p-4 flex flex-col gap-2 bg-teal-600" style={{ boxShadow: '0 1px 2px rgba(0,0,0,0.2), 0 2px 6px rgba(0,0,0,0.12)' }}>
...
function TurnaroundCard({ avgTurnaround }: { avgTurnaround: string | null }) {
  return (
    <div className="rounded-2xl p-4 flex flex-col gap-2 bg-orange-500" style={{ boxShadow: '0 1px 2px rgba(0,0,0,0.2), 0 2px 6px rgba(0,0,0,0.12)' }}>
